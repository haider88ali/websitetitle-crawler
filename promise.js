        ...
        const lib = fullUrl.startsWith('https') ? https : http;
            ...
            lib.get(fullUrl,(res) => {
                ...
                res.on('end' ,() => {
                    ...
                    resolve({ url: address, title: match ? match[1] : 'NO RESPONSE' });
        
                });
            }).on('error',() => {
                resolve({ url: address, title: 'NO RESPONSE' });
...
//   return res.send("here");
  ...
  Promise.all(list.map(fetchTitle)).then(results => {
...
}).listen(3000, () => console.log('Async version running at http://localhost:3000'));
